defmodule MyHeadsUp.Responses do
  @moduledoc """
  ...
  def subscribe_responses(%Scope{} = scope) do
    ...
    Phoenix.PubSub.subscribe(MyHeadsUp.PubSub, "user:#{key}:responses")
  end

  defp broadcast(%Scope{} = scope, message) do
    key = scope.user.id

    Phoenix.PubSub.broadcast(MyHeadsUp.PubSub, "user:#{key}:responses", message)
  end
  ...
  def update_response(%Scope{} = scope, %Response{} = response, attrs) do
    ...
    with {:ok, response = %Response{}} <-
           ...
           |> Response.changeset(attrs, scope)
           |> Repo.update() do
      broadcast(scope, {:updated, response})
      {:ok, response}
    end
  end
  ...
  def delete_response(%Scope{} = scope, %Response{} = response) do
    ...
    with {:ok, response = %Response{}} <-
      ...
      broadcast(scope, {:deleted, response})
      {:ok, response}
    end
  end
